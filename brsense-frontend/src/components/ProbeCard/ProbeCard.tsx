...
const getProgressColor = (value: number, point: MapPoint) => {
  const v1 =
    point.config_moisture_v1 ?? (point.config_min ? point.config_min - 10 : 30);
...
export function ProbeCard({
  ...
  const { profileData, lastCommunicationDate, rainStats } =
    useProbeStats(point);

  const handleTabChange = (tab: TabType) => {
    const nextTab =
      tab === "copiloto" && activeTab === "copiloto" ? "grafico" : tab;
    ...
    if (isFrontVisible) {
      setBackContent(nextTab);
    } else {
      setFrontContent(nextTab);
    }
            ...
            profileData.map((r) => {
              ...
              return (
                <Box
                  ...
                  onClick={() =>
                    onSelectDepthRef &&
                    onSelectDepthRef(isSelected ? null : r.depth_cm)
                  }
                  cursor="pointer"
...
const RainBox = ({ label, value, isHighlight }: RainBoxProps) => (
  ...
  >
    <Text fontSize={{ base: "xs" }} color="gray.400">
      {label}
    </Text>
    <Text
      fontWeight="bold"
      fontSize={{ base: "sm", md: "md" }}
      color={isHighlight ? "cyan.200" : "gray.200"}
    >
      {value?.toFixed(1) || "0.0"}{" "}
